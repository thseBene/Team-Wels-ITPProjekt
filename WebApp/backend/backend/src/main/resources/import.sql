-- Import / Schema-Skript (erweitert um Mitarbeiter-Verwaltung + Verifizierung)
...
-- Benutzer (Basis-Tabelle für alle User-Typen) + Verifizierung
CREATE TABLE benutzer (
    ...
    -- E-Mail-Verifizierung
                          ...
                          email_verification_token_created_at TIMESTAMP WITHOUT TIME ZONE,

    -- Telefon-Verifizierung (6-stelliger Code)
                          tel_verified BOOLEAN NOT NULL DEFAULT FALSE,
                          tel_verification_code VARCHAR(10),
                          tel_verification_code_created_at TIMESTAMP WITHOUT TIME ZONE,
...
-- Notification Tabelle (vereinfacht)
CREATE TABLE notification (
                              id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                              benutzer_id BIGINT NOT NULL REFERENCES benutzer(id) ON DELETE CASCADE,
                              feedback_id BIGINT REFERENCES feedback(id) ON DELETE CASCADE,
                              typ VARCHAR(50) NOT NULL,  -- EMAIL oder SMS
                              gelesen BOOLEAN NOT NULL DEFAULT FALSE,
                              sent BOOLEAN NOT NULL DEFAULT FALSE,
                              created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT NOW(),
                              sent_at TIMESTAMP WITHOUT TIME ZONE,
                              old_status VARCHAR(50),  -- Für STATUS_CHANGED
                              new_status VARCHAR(50),  -- Für STATUS_CHANGED
                              CONSTRAINT notification_benutzer_fk FOREIGN KEY (benutzer_id) REFERENCES benutzer(id) ON DELETE CASCADE,
                              CONSTRAINT notification_feedback_fk FOREIGN KEY (feedback_id) REFERENCES feedback(id) ON DELETE CASCADE
);


-- Indexes
CREATE INDEX idx_notification_benutzer ON notification(benutzer_id);
CREATE INDEX idx_notification_feedback ON notification(feedback_id);
...
-- NEU: Indexes für Verifizierung
CREATE INDEX idx_benutzer_email_verification_token ON benutzer(email_verification_token);
CREATE INDEX idx_benutzer_tel_verification_code ON benutzer(tel_verification_code);
CREATE INDEX idx_benutzer_email_verified ON benutzer(email_verified);
CREATE INDEX idx_benutzer_tel_verified ON benutzer(tel_verified);
...
-- Kategorien (Rest bleibt wie gehabt...)
-- -------------------------
-- Beispiel-Daten einfügen
-- -------------------------
